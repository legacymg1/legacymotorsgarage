...
function renderMsgs(){
  ...
  const RLANG=(localStorage.getItem('lmg_lang_pref')||'es'), isGrp=(curCh==='group');   // group chat se lee en el idioma de cada quien
  box.innerHTML=arr.map(m=>{ const mine=(m.byEmail||'')===ME; const when=m.ts?new Date(m.ts).toLocaleString('es-MX',{month:'short',day:'numeric',hour:'numeric',minute:'2-digit'}):'';
    const disp=isGrp?((RLANG==='en')?(m.textEn||m.text||''):(m.textEs||m.text||'')):(m.text||'');
